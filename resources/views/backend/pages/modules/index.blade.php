                    ...
                    @foreach ($categoryModules as $module)
                        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                            <div class="flex justify-between">
                                ...
                                <button id="dropdownMenuIconButton" data-dropdown-toggle="dropdownMore-{{ $module->id }}" class="inline-flex items-right h-9 p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600" type="button">
                                    <i class="bi bi-three-dots-vertical"></i>
                                </button>

                                <div id="dropdownMore-{{ $module->id }}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton">
                                        <li>
                                            <button
                                                x-data="{ showDeleteModal: false }"
                                                data-modal-target="delete-modal-{{ $module->id }}" data-modal-toggle="delete-modal-{{ $module->id }}"
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                ...
                                @foreach ($module->tags as $tag)
                                    <span class="inline-block px-2 py-1 text-xs font-medium text-white bg-gray-400 rounded-full mr-1">{{ $tag }}</span>
                                @endforeach
                            </p>
                        ...
                        <!-- Delete modal -->
                        <div id="delete-modal-{{ $module->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full z-99999">
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                <button type="button" class="absolute top-3 right-3 text-gray-400 hover:text-gray-900 dark:hover:text-white" data-modal-hide="delete-modal-{{ $module->id }}">
                                ...
                                <div class="p-6 text-center">
                                    ...
                                    <form x-ref="deleteForm{{ $module->id }}" action="{{ route('admin.modules.delete', $module->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
